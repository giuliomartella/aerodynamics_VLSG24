...
cd_values_wing = zeros(size(alpha_range));
cd2d_wing = zeros(length(alpha_range), wing.discretize(2));
...
if nargin == 3
    ...
    cd_values_tail = zeros(size(alpha_range));
    cd2d_tail = zeros(length(alpha_range), tail.discretize(2));
...
for i = 1:length(alpha_range)
    ...
    cd_values_wing(i) = wing.cD;
    cd2d_wing(i, :) = wing.cD2D;
    ...
    if nargin == 3
        ...
        cd_values_tail(i) = tail.cD;
        cd2d_tail(i, :) = tail.cD2D;
    end
end
...
% Wing drag polar
subplot(3, 2, 5);
plot(cd_values_wing, cl_values_wing, 'o-', 'LineWidth', 2);
xlabel('C_D');
ylabel('C_L');
title('Wing Drag Polar');
grid on;

% Wing drag distribution
subplot(3, 2, 6);
for i = 1:length(alpha_range)
    plot(span_wing, cd2d_wing(i, :), 'LineWidth', 2);
    hold on;
end
xlabel('Span (m)');
ylabel('C_D2D');
title('Wing Drag Distribution');
grid on;
