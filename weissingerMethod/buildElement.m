...
%
% This function plots the points along the quarter chord line of the wing 
% surface and overlays the normals as vectors for visualization.
%
% Input:
%   - wing: a structure containing wing geometry data, including:
%       * wing.xQuarterChord: x-coordinates of the quarter chord points.
%       * wing.yQuarterChord: y-coordinates of the quarter chord points.
%       * wing.zQuarterChord: z-coordinates of the quarter chord points.
%       * wing.normals: a matrix (Nx3) containing normal vectors at each quarter chord point.
%
% Example:
%   plotQuarterChordWithNormals(wing);

    % Extract data from the wing structure
    xQuarterChord = wing.quarter(:,1);
    yQuarterChord = wing.quarter(:,2);
    zQuarterChord = wing.quarter(:,3);
    normals = wing.quarterNormal;

    % Verify dimensions consistency
    if length(xQuarterChord) ~= length(yQuarterChord) || length(xQuarterChord) ~= length(zQuarterChord)
        error('Quarter chord points dimensions are inconsistent.');
    end
    if size(normals, 1) ~= length(xQuarterChord) || size(normals, 2) ~= 3
        error('Normal vectors dimensions are inconsistent.');
    end

    % Plot quarter chord points
    figure(5);
    plot3(xQuarterChord, yQuarterChord, zQuarterChord, 'o-', 'DisplayName', 'Quarter Chord Line');
    hold on;

    % Add normals to the plot
    quiver3(xQuarterChord, yQuarterChord, zQuarterChord, ...
